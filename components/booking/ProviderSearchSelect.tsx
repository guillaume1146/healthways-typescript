...
const ACCENT_CLASSES: Record<string, { selected: string; tag: string; check: string; focus: string; bg: string }> = {
  blue: { selected: 'bg-blue-100 border-blue-500 ring-2 ring-blue-300', tag: 'bg-blue-100 text-blue-700', check: 'text-blue-500', focus: 'focus:border-blue-500', bg: 'from-blue-50 to-indigo-50' },
  pink: { selected: 'bg-pink-100 border-pink-500 ring-2 ring-pink-300', tag: 'bg-pink-100 text-pink-700', check: 'text-pink-500', focus: 'focus:border-pink-500', bg: 'from-pink-50 to-purple-50' },
  purple: { selected: 'bg-purple-100 border-purple-500 ring-2 ring-purple-300', tag: 'bg-green-100 text-green-700', check: 'text-purple-500', focus: 'focus:border-purple-500', bg: 'from-pink-50 to-purple-50' },
  cyan: { selected: 'bg-cyan-100 border-cyan-500 ring-2 ring-cyan-300', tag: 'bg-cyan-100 text-cyan-700', check: 'text-cyan-500', focus: 'focus:border-cyan-500', bg: 'from-cyan-50 to-blue-50' },
}
      ...
      {selected && !isOpen && (
        <div className={`p-3 border rounded-lg ${accent.selected} mb-2`}>
          <div className="flex items-center gap-3">
            <div className={`w-10 h-10 bg-gradient-to-r ${avatarGradient} rounded-lg flex items-center justify-center text-white font-bold text-sm flex-shrink-0`}>
                ...
                filtered.map((provider) => (
                  <button
                    ...
                    className={`w-full p-3 text-left hover:bg-gray-50 transition border-b border-gray-100 last:border-0 ${
                      selectedId === provider.userId ? `bg-gradient-to-r ${accent.bg}` : ''
                    }`}
                  >
                    <div className="flex items-center gap-3">
                      <div className={`w-10 h-10 bg-gradient-to-r ${avatarGradient} rounded-lg flex items-center justify-center text-white font-bold text-sm flex-shrink-0`}>
