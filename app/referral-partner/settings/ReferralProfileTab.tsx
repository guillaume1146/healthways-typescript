...
const ReferralProfileTab: React.FC = () => {
  const [profile, setProfile] = useState<ProfileData>({
    firstName: '', lastName: '', email: '', phone: '', address: '',
    businessType: '', commissionRate: '', referralCode: '',
  })
  ...
  const loadProfile = useCallback(async () => {
    try {
      ...
      const p = data.profile || data.referralPartnerProfile || {}

      setProfile({
        firstName: data.firstName || '', lastName: data.lastName || '',
        email: data.email || '', phone: data.phone || '', address: data.address || '',
        businessType: p.businessType || '', commissionRate: p.commissionRate?.toString() || '',
      ...
      const res = await fetch(`/api/users/${userId}`, {
        ...
        body: JSON.stringify({
          ...
          profileData: {
            ...
            commissionRate: profile.commissionRate ? parseFloat(profile.commissionRate) : null,
  ...
  return (
    <form className="space-y-8" onSubmit={handleSave}>
      ...
      <div className="border-t pt-6">
        <h3 className="text-lg font-bold text-gray-800 mb-4">Referral Partner Details</h3>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div><label className="block text-sm font-medium text-gray-700 mb-1">Business Type</label><input name="businessType" value={profile.businessType} onChange={handleChange} placeholder="e.g. Clinic, Hospital, Pharmacy" className="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-200 focus:border-blue-500" /></div>
          <div><label className="block text-sm font-medium text-gray-700 mb-1">Commission Rate (%)</label><input name="commissionRate" type="number" step="0.1" value={profile.commissionRate} onChange={handleChange} className="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-200 focus:border-blue-500" /></div>
          <div className="md:col-span-2">
            <label className="block text-sm font-medium text-gray-700 mb-1">Referral Code</label>
            <input name="referralCode" value={profile.referralCode} readOnly className="w-full px-4 py-2.5 border border-gray-300 rounded-lg bg-gray-50 text-gray-600 cursor-not-allowed" />
            <p className="text-xs text-gray-500 mt-1">Your referral code is auto-generated and cannot be changed.</p>
          </div>
        </div>
      </div>
