...
export async function PATCH(
  ...
  try {
    if (action === 'accept') {
      const result = await acceptBooking(id, 'lab-test', auth.sub)
      return NextResponse.json({ success: true, ...result })
    } else if (action === 'cancel') {
      const result = await cancelBooking(id, 'lab-test', auth.sub, reason)
      return NextResponse.json({ success: true, ...result })
    } else {
      await denyBooking(id, 'lab-test', auth.sub)
    ...
    console.error('PATCH /api/bookings/lab-test/[id] error:', error)
