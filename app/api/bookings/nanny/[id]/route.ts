...
export async function PATCH(
  ...
  try {
    if (action === 'accept') {
      const result = await acceptBooking(id, 'nanny', auth.sub)
      return NextResponse.json({ success: true, ...result })
    } else if (action === 'cancel') {
      const result = await cancelBooking(id, 'nanny', auth.sub, reason)
      return NextResponse.json({ success: true, ...result })
    } else {
      await denyBooking(id, 'nanny', auth.sub)
    ...
    console.error('PATCH /api/bookings/nanny/[id] error:', error)
