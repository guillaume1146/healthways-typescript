...
import { FaCheckCircle, FaArrowLeft, FaCalendarAlt, FaClock, FaUserNurse, FaWallet } from 'react-icons/fa'
...
export default function BookNursePage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = use(params)
    ...
    return (
      <div className="max-w-2xl mx-auto">
        <div className="bg-white rounded-2xl p-8 shadow-lg text-center">
          ...
          <p className="text-gray-600 mb-8">Your nurse consultation has been successfully booked.</p>

          {/* Ticket Card */}
          <div className="bg-gradient-to-r from-pink-600 to-rose-700 rounded-2xl p-6 text-white mb-8 text-left">
            <div className="flex items-center justify-between mb-6">
              <div>
                <h3 className="text-lg font-bold">Nurse Booking Ticket</h3>
                <p className="text-pink-200 text-sm">Keep this for your records</p>
              </div>
              <div className="text-right">
                <p className="text-pink-200 text-xs">Ticket ID</p>
            ...
            <div className="grid grid-cols-2 gap-4 text-sm">
              <div>
                <p className="text-pink-200 mb-1">Nurse</p>
                <p className="font-semibold">{nurse.firstName} {nurse.lastName}</p>
              </div>
              <div>
                <p className="text-pink-200 mb-1">Specialization</p>
                <p className="font-semibold">{nurse.specialization[0] || 'General Nursing'}</p>
              </div>
              <div className="flex items-center gap-2">
                <FaCalendarAlt className="text-pink-200" />
                <div>
                  <p className="text-pink-200 text-xs">Date</p>
              ...
              <div className="flex items-center gap-2">
                ...
                <div>
                  <p className="text-pink-200 text-xs">Time</p>
                  <p className="font-semibold">{submitData?.scheduledTime || ''}</p>
                </div>
              </div>
              <div className="flex items-center gap-2">
                <FaUserNurse className="text-pink-200" />
                <div>
                  <p className="text-pink-200 text-xs">Type</p>
            ...
            {submitData?.reason && (
              <div className="mt-4 pt-4 border-t border-white/20">
                <p className="text-pink-200 text-xs">Reason</p>
          ...
          <div className="flex flex-col sm:flex-row gap-3">
            <Link
              href="/patient/dashboard"
              className="flex-1 bg-gradient-to-r from-pink-600 to-pink-700 text-white py-3 px-6 rounded-lg font-semibold hover:from-pink-700 hover:to-pink-800 transition-all text-center"
            >
              Back to Dashboard
            </Link>
            <Link
