...
const InsuranceProfileTab: React.FC = () => {
  const [profile, setProfile] = useState<ProfileData>({
    firstName: '', lastName: '', email: '', phone: '', address: '',
    companyName: '', licenseNumber: '', coverageTypes: '',
  })
  ...
  const loadProfile = useCallback(async () => {
    try {
      ...
      const p = data.profile || data.insuranceRepProfile || {}

      setProfile({
        firstName: data.firstName || '', lastName: data.lastName || '',
        email: data.email || '', phone: data.phone || '', address: data.address || '',
        companyName: p.companyName || '', licenseNumber: p.licenseNumber || '',
        coverageTypes: Array.isArray(p.coverageTypes) ? p.coverageTypes.join(', ') : (p.coverageTypes || ''),
      })
      ...
      const res = await fetch(`/api/users/${userId}`, {
        ...
        body: JSON.stringify({
          ...
          profileData: {
            ...
            coverageTypes: profile.coverageTypes.split(',').map(s => s.trim()).filter(Boolean),
          },
        }),
      })
  ...
  return (
    <form className="space-y-8" onSubmit={handleSave}>
      ...
      <div className="border-t pt-6">
        <h3 className="text-lg font-bold text-gray-800 mb-4">Insurance Details</h3>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div className="md:col-span-2"><label className="block text-sm font-medium text-gray-700 mb-1">Coverage Types (comma-separated)</label><input name="coverageTypes" value={profile.coverageTypes} onChange={handleChange} placeholder="e.g. Health, Life, Motor, Property" className="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-200 focus:border-blue-500" /></div>
        </div>
      </div>
...
export default InsuranceProfileTab
