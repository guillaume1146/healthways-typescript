...
const ResponderProfileTab: React.FC = () => {
  const [profile, setProfile] = useState<ProfileData>({
    firstName: '', lastName: '', email: '', phone: '', address: '',
    certifications: '', vehicleType: '', responseZone: '', emtLevel: '',
  })
  ...
  const loadProfile = useCallback(async () => {
    try {
      ...
      const p = data.profile || data.emergencyWorkerProfile || {}

      setProfile({
        firstName: data.firstName || '', lastName: data.lastName || '',
        email: data.email || '', phone: data.phone || '', address: data.address || '',
        certifications: Array.isArray(p.certifications) ? p.certifications.join(', ') : (p.certifications || ''),
        vehicleType: p.vehicleType || '', responseZone: p.responseZone || '', emtLevel: p.emtLevel || '',
      })
      ...
      const res = await fetch(`/api/users/${userId}`, {
        ...
        body: JSON.stringify({
          ...
          profileData: {
            ...
            vehicleType: profile.vehicleType, responseZone: profile.responseZone, emtLevel: profile.emtLevel,
          },
        }),
      })
  ...
  return (
    <form className="space-y-8" onSubmit={handleSave}>
      ...
      <div className="border-t pt-6">
        <h3 className="text-lg font-bold text-gray-800 mb-4">Emergency Service Details</h3>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div><label className="block text-sm font-medium text-gray-700 mb-1">Certifications (comma-separated)</label><input name="certifications" value={profile.certifications} onChange={handleChange} placeholder="e.g. ALS, PALS, ACLS, Hazmat" className="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-200 focus:border-blue-500" /></div>
          <div><label className="block text-sm font-medium text-gray-700 mb-1">Vehicle Type</label><input name="vehicleType" value={profile.vehicleType} onChange={handleChange} placeholder="e.g. ALS Ambulance" className="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-200 focus:border-blue-500" /></div>
          <div><label className="block text-sm font-medium text-gray-700 mb-1">Response Zone</label><input name="responseZone" value={profile.responseZone} onChange={handleChange} placeholder="e.g. Port Louis & Surrounding Areas" className="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-200 focus:border-blue-500" /></div>
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">EMT Level</label>
            <select name="emtLevel" value={profile.emtLevel} onChange={handleChange} className="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-200 focus:border-blue-500">
              <option value="">Select level...</option>
              <option value="EMR">Emergency Medical Responder (EMR)</option>
...
export default ResponderProfileTab
