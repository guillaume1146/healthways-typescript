...
const NannyProfileTab: React.FC = () => {
  ...
  const loadProfile = useCallback(async () => {
    try {
      ...
      const p = data.profile || data.nannyProfile || {}
  ...
  return (
    <form className="space-y-8" onSubmit={handleSave}>
      {saveStatus && (
        <div className={`px-4 py-3 rounded-lg text-sm font-medium ${saveStatus.type === 'success' ? 'bg-green-50 text-green-800 border border-green-200' : 'bg-red-50 text-red-800 border border-red-200'}`}>{saveStatus.message}</div>
      )}
      <div>
      ...
      <div className="border-t pt-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div><label className="block text-sm font-medium text-gray-700 mb-1">Years of Experience</label><input name="experience" type="number" value={profile.experience} onChange={handleChange} className="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-200 focus:border-blue-500" /></div>
          <div><label className="block text-sm font-medium text-gray-700 mb-1">Certifications (comma-separated)</label><input name="certifications" value={profile.certifications} onChange={handleChange} placeholder="e.g. CPR, First Aid, Early Childhood Education" className="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-200 focus:border-blue-500" /></div>
        </div>
      </div>
...
export default NannyProfileTab
