...
export default function NannyReviewsPage() {
    ...
    const fetchReviews = async () => {
      try {
        ...
        if (res.ok) {
          ...
          if (data.success && data.data?.nannyProfile?.reviews) {
            setReviews(data.data.nannyProfile.reviews)
          }
        }
      } catch (error) {
  ...
  return (
    <div className="space-y-6">
      {/* Header */}
      <div className="bg-gradient-to-r from-purple-500 via-purple-600 to-pink-500 rounded-2xl p-5 sm:p-6 text-white">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div>
            ...
            <p className="text-purple-100 text-sm mt-1">Feedback from the families you serve</p>
          </div>
          <div className="text-center">
            <p className="text-4xl font-bold">{averageRating.toFixed(1)}</p>
            <div className="flex justify-center mt-1">{renderStars(averageRating, 'text-sm')}</div>
            <p className="text-purple-100 text-xs mt-1">{reviews.length} review{reviews.length !== 1 ? 's' : ''}</p>
          </div>
        </div>
      </div>
      ...
      {filteredReviews.length === 0 ? (
        <div className="bg-white rounded-2xl p-12 text-center shadow-sm border border-gray-200">
          ...
          <p className="text-gray-500 text-sm">
            {reviews.length === 0
              ? 'Reviews from families will appear here after they complete their childcare bookings with you.'
          ...
          {filteredReviews.map((review) => (
            ...
            >
              <div className="flex items-start gap-4">
                <div className="w-12 h-12 bg-gradient-to-br from-purple-500 to-pink-500 rounded-full flex items-center justify-center text-white font-bold text-sm flex-shrink-0">
                  ...
                  <div className="flex items-center justify-between mb-1">
                    <div>
                      <h4 className="font-semibold text-gray-900 text-sm sm:text-base">
                        {review.patientName}
                        {review.verified && (
                          <span className="ml-2 text-xs text-purple-600 bg-purple-50 px-2 py-0.5 rounded-full">Verified</span>
                        )}
                      </h4>
