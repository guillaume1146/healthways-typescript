...
import { FaPlus, FaEdit, FaTrash, FaSearch, FaTimes, FaBaby } from 'react-icons/fa';

interface NannyService {
  id: string;
  serviceName: string;
  category: string;
  description?: string;
...
type NannyServiceFormData = Omit<NannyService, 'id'>;
...
const emptyForm: NannyServiceFormData = {
  serviceName: '',
  category: 'Daily Care',
  description: '',
  price: 0,
...
export default function NannyServicesPage() {
  const [services, setServices] = useState<NannyService[]>([]);
  ...
  const [formData, setFormData] = useState<NannyServiceFormData>(emptyForm);
  const [submitting, setSubmitting] = useState(false);

  const fetchServices = useCallback(async () => {
    try {
      setLoading(true);
      const res = await fetch('/api/nanny/services');
  ...
  const openEditModal = (service: NannyService) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingId) {
        const res = await fetch(`/api/nanny/services/${editingId}`, {
          method: 'PUT',
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      const res = await fetch(`/api/nanny/services/${id}`, {
        method: 'DELETE',
      });
    ...
    <div className="p-6 max-w-7xl mx-auto">
      <div className="flex items-center justify-between mb-8">
        <div className="flex items-center gap-3">
          <FaBaby className="text-3xl text-purple-600" />
            ...
            <table className="w-full">
              <thead>
                <tr className="bg-gray-50 border-b border-gray-200">
                  ...
                  <th className="text-left px-6 py-3 text-xs font-semibold text-gray-500 uppercase tracking-wider">Age Range</th>
                ...
                {filteredServices.length === 0 ? (
                  ...
                  filteredServices.map((service) => (
                    <tr key={service.id} className="hover:bg-gray-50 transition-colors">
                      ...
                      <td className="px-6 py-4 text-gray-700">{service.ageRange || 'All ages'}</td>
          ...
          <div className="relative bg-white rounded-xl shadow-xl w-full max-w-2xl max-h-[90vh] overflow-y-auto mx-4">
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-5">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <input
                    ...
                    placeholder="e.g. 0-2 years, 3-6 years, All ages"
                    className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-transparent outline-none"
                  />
                </div>
              </div>
              ...
              <div className="flex justify-end gap-3 pt-2">
                ...
                <button type="submit" disabled={submitting} className="px-4 py-2 bg-purple-600 hover:bg-purple-700 text-white rounded-lg transition-colors font-medium disabled:opacity-50 disabled:cursor-not-allowed">
