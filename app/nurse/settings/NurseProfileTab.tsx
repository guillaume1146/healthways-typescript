...
const NurseProfileTab: React.FC = () => {
  ...
  const loadProfile = useCallback(async () => {
    try {
      ...
      const p = data.profile || data.nurseProfile || {}
  ...
  return (
    <form className="space-y-8" onSubmit={handleSave}>
      {saveStatus && (
        <div className={`px-4 py-3 rounded-lg text-sm font-medium ${saveStatus.type === 'success' ? 'bg-green-50 text-green-800 border border-green-200' : 'bg-red-50 text-red-800 border border-red-200'}`}>
          {saveStatus.message}
        </div>
      )}
      ...
      <div className="border-t pt-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div>
            ...
            <input name="experience" type="number" value={profile.experience} onChange={handleChange} className="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-200 focus:border-blue-500" />
          </div>
          <div className="md:col-span-2">
            <label className="block text-sm font-medium text-gray-700 mb-1">Specializations (comma-separated)</label>
            <input name="specializations" value={profile.specializations} onChange={handleChange} placeholder="e.g. Pediatric Nursing, Critical Care, Home Health" className="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-200 focus:border-blue-500" />
          </div>
        </div>
      </div>
