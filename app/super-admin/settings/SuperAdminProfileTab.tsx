...
const SuperAdminProfileTab: React.FC = () => {
  ...
  const loadProfile = useCallback(async () => {
    try {
      ...
      const p = data.profile || data.regionalAdminProfile || {}

      setProfile({
        firstName: data.firstName || '', lastName: data.lastName || '',
        email: data.email || '', phone: data.phone || '', address: data.address || '',
        region: p.region || '', country: p.country || '', countryCode: p.countryCode || '',
      })
    ...
    try {
      const res = await fetch(`/api/users/${userId}`, {
        ...
        body: JSON.stringify({
          ...
          profileData: {
            region: profile.region, country: profile.country, countryCode: profile.countryCode,
          },
        }),
      })
  ...
  return (
    <form className="space-y-8" onSubmit={handleSave}>
      ...
      <div className="border-t pt-6">
        <h3 className="text-lg font-bold text-gray-800 mb-4">Regional Administration</h3>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div><label className="block text-sm font-medium text-gray-700 mb-1">Region</label><input name="region" value={profile.region} onChange={handleChange} placeholder="e.g. East Africa" className="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-200 focus:border-blue-500" /></div>
          <div><label className="block text-sm font-medium text-gray-700 mb-1">Country</label><input name="country" value={profile.country} onChange={handleChange} placeholder="e.g. Mauritius" className="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-200 focus:border-blue-500" /></div>
          <div><label className="block text-sm font-medium text-gray-700 mb-1">Country Code</label><input name="countryCode" value={profile.countryCode} onChange={handleChange} placeholder="e.g. MU" className="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-200 focus:border-blue-500" /></div>
        </div>
      </div>
