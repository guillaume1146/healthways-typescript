...
const LabTechProfileTab: React.FC = () => {
  const [profile, setProfile] = useState<ProfileData>({
    firstName: '', lastName: '', email: '', phone: '', address: '',
    licenseNumber: '', labName: '', specializations: '',
  })
  ...
  const loadProfile = useCallback(async () => {
    try {
      ...
      const p = data.profile || data.labTechProfile || {}

      setProfile({
        firstName: data.firstName || '', lastName: data.lastName || '',
        email: data.email || '', phone: data.phone || '', address: data.address || '',
        licenseNumber: p.licenseNumber || '', labName: p.labName || '',
    ...
    try {
      const res = await fetch(`/api/users/${userId}`, {
        ...
        body: JSON.stringify({
          ...
          profileData: {
            licenseNumber: profile.licenseNumber, labName: profile.labName,
  ...
  return (
    <form className="space-y-8" onSubmit={handleSave}>
      ...
      <div className="border-t pt-6">
        <h3 className="text-lg font-bold text-gray-800 mb-4">Laboratory Details</h3>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div><label className="block text-sm font-medium text-gray-700 mb-1">Lab Name</label><input name="labName" value={profile.labName} onChange={handleChange} className="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-200 focus:border-blue-500" /></div>
          <div className="md:col-span-2"><label className="block text-sm font-medium text-gray-700 mb-1">Specializations (comma-separated)</label><input name="specializations" value={profile.specializations} onChange={handleChange} placeholder="e.g. Hematology, Microbiology, Biochemistry" className="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-200 focus:border-blue-500" /></div>
        </div>
      </div>
...
export default LabTechProfileTab
