...
import { FaCheck, FaTimes, FaSpinner, FaCalendarAlt, FaClock, FaEnvelope, FaPhone, FaFlask, FaExclamationCircle } from 'react-icons/fa'
...
export default function LabTechBookingRequestsPage() {
  ...
  const fetchBookings = useCallback(async () => {
    try {
      ...
      const res = await fetch(`/api/lab-techs/${uid}/booking-requests`)
                ...
                <div className="flex-1 space-y-3">
                  ...
                  <div className="flex flex-wrap items-center gap-3 text-sm">
                    <span className="inline-flex items-center rounded-full bg-cyan-100 text-cyan-800 px-2.5 py-0.5 text-xs font-medium">
                      <FaFlask className="mr-1" /> {booking.testName}
                    </span>
                    ...
                    {booking.price && (
                      <span className="text-gray-500">Rs {booking.price}</span>
                    )}
                  </div>

                  {booking.sampleType && (
                    <p className="text-sm text-gray-600">
                      <span className="font-medium">Sample Type:</span> {booking.sampleType}
                    </p>
                  )}
