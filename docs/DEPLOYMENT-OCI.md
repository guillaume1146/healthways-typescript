# Healthwyz — Deployment Guide (Oracle Cloud Free Tier)

Deploy Healthwyz on an Oracle Cloud Infrastructure (OCI) **Always Free** Arm-based VM with Docker Compose, Nginx, and automated CI/CD.
...
**Recommended for Healthwyz**: 1 VM with 2 OCPUs + 12 GB RAM — more than enough.
...
```bash
ssh-keygen -t ed25519 -C "healthwyz-oci" -f ~/.ssh/oci_healthwyz
```
...
| **SSH key** | Upload your `~/.ssh/oci_healthwyz.pub` |
...
oci compute instance launch \
  ...
  --ssh-authorized-keys-file ~/.ssh/oci_healthwyz.pub \
  --display-name healthwyz-vm
```
...
```bash
ssh -i ~/.ssh/oci_healthwyz ubuntu@YOUR_VM_PUBLIC_IP
```
...
sudo tee /opt/healthwyz-backups/backup.sh << 'SCRIPT'
#!/bin/bash
...
# Create a bucket
oci os bucket create --name healthwyz-backups --compartment-id YOUR_COMPARTMENT_OCID

# Upload backup
oci os object put --bucket-name healthwyz-backups --file /opt/healthwyz-backups/backup_TIMESTAMP.sql
```
...
| `OCI_SSH_PRIVATE_KEY` | Your SSH private key | Contents of `~/.ssh/oci_healthwyz` |

### How to add the SSH private key secret

```bash
# Copy the private key content to clipboard
cat ~/.ssh/oci_healthwyz
```
...
```bash
ssh -i ~/.ssh/oci_healthwyz ubuntu@YOUR_VM_IP
cd ~/healthways-typescript
...
| SSH connection refused | Verify security list allows port 22, check `~/.ssh/oci_healthwyz` permissions (`chmod 600`) |
