...
- [Regions](#regions)
- [Mauritius (REG-MU)](#mauritius-reg-mu)
- [Madagascar (REG-MG)](#madagascar-reg-mg)
- [Kenya (REG-KE)](#kenya-reg-ke)
- [Togo (REG-TG)](#togo-reg-tg)
- [Benin (REG-BJ)](#benin-reg-bj)
- [Rwanda (REG-RW)](#rwanda-reg-rw)
- [Super Admin](#super-admin)
- [Registration Test Scenarios](#registration-test-scenarios)
- [Fake Test Documents](#fake-test-documents)

---

## Regions

| Region ID | Name | Country Code | Language | Flag |
|-----------|------|:------------:|----------|------|
| REG-MU | Mauritius | MU | en | 🇲🇺 |
| REG-MG | Madagascar | MG | fr | 🇲🇬 |
| REG-KE | Kenya | KE | en | 🇰🇪 |
| REG-TG | Togo | TG | fr | 🇹🇬 |
| REG-BJ | Benin | BJ | fr | 🇧🇯 |
| REG-RW | Rwanda | RW | en | 🇷🇼 |
...
> All multi-country users use password: **User123!**
...
Created from environment variables on server startup.
...
Use the fake documents from `docs/test-documents/` for all uploads below.

### Scenario 1: Register a Patient (Mauritius)

| Step | Field | Value |
|------|-------|-------|
| 1 — Account Type | User Type | Patient |
| 2 — Basic Info | Full Name | Tania Doobur |
| | Email | tania.doobur@test.com |
| | Password | TestPatient123! |
| | Confirm Password | TestPatient123! |
| | Phone | +230 5999 0001 |
| | Date of Birth | 1992-05-15 |
| | Gender | Female |
| | Address | 45 Rue de la Paix, Curepipe, Mauritius |
| | Region | Mauritius |
| | Profile Picture | Upload `docs/test-documents/profile-photo.svg` |
| 3 — Documents | National ID/Passport (required) | Upload `docs/test-documents/national-id.svg` |
| | Health Insurance Card (optional) | Upload `docs/test-documents/insurance-card.svg` |
| | Proof of Address (required) | Upload `docs/test-documents/proof-of-address.svg` |
| | Medical History (optional) | Skip |
| 4 — Verify & Submit | Accept Terms | Yes |

**After registration:** Login with `tania.doobur@test.com` / `TestPatient123!` → go to Profile → Documents tab → verify all 3 documents appear.

---

### Scenario 2: Register a Doctor (Kenya)

| Step | Field | Value |
|------|-------|-------|
| 1 — Account Type | User Type | Doctor |
| 2 — Basic Info | Full Name | Daniel Ochieng |
| | Email | daniel.ochieng@test.ke |
| | Password | TestDoctor123! |
| | Confirm Password | TestDoctor123! |
...
| | Profile Picture | Upload `docs/test-documents/profile-photo.svg` |
| 3 — Documents | National ID/Passport (required) | Upload `docs/test-documents/national-id.svg` |
| | Medical Degree (required) | Upload `docs/test-documents/medical-degree.svg` |
| | Professional License (required) | Upload `docs/test-documents/professional-license.svg` |
| | Registration Certificate (required) | Upload `docs/test-documents/work-certificate.svg` |
| | Work Certificate (optional) | Upload `docs/test-documents/work-certificate.svg` |
| 4 — Verify & Submit | Accept Terms | Yes |

**After registration:** Login → Profile → verify 5 documents and profile picture. Go to Feed → suggestions should show Kenyan users first.

---

### Scenario 3: Register a Nurse (Madagascar)

| Step | Field | Value |
|------|-------|-------|
| 1 — Account Type | User Type | Nurse |
| 2 — Basic Info | Full Name | Fara Andriantsoa |
| | Email | fara.andriantsoa@test.mg |
| | Password | TestNurse123! |
| | Confirm Password | TestNurse123! |
| | Phone | +261 34 12 345 |
| | Date of Birth | 1990-03-08 |
| | Gender | Female |
| | Address | Lot 123, Antsahavola, Antananarivo |
| | Region | Madagascar |
| | Profile Picture | Upload `docs/test-documents/profile-photo.svg` |
| 3 — Documents | National ID/Passport (required) | Upload `docs/test-documents/national-id.svg` |
| | Nursing Degree/Diploma (required) | Upload `docs/test-documents/medical-degree.svg` |
| | Professional License (required) | Upload `docs/test-documents/professional-license.svg` |
| | Registration Certificate (required) | Upload `docs/test-documents/work-certificate.svg` |
...
### Scenario 4: Register a Nanny (Mauritius)

| Step | Field | Value |
|------|-------|-------|
| 1 — Account Type | User Type | Nanny |
| 2 — Basic Info | Full Name | Reshma Doobur |
| | Email | reshma.doobur@test.com |
| | Password | TestNanny123! |
| | Confirm Password | TestNanny123! |
| | Phone | +230 5888 0002 |
...
| | Profile Picture | Upload `docs/test-documents/profile-photo.svg` |
| 3 — Documents | National ID/Passport (required) | Upload `docs/test-documents/national-id.svg` |
| | Police Clearance Certificate (required) | Upload `docs/test-documents/police-clearance.svg` |
| | Childcare Training Certificate (optional) | Upload `docs/test-documents/work-certificate.svg` |
| | Employment References (optional) | Skip |
| 4 — Verify & Submit | Accept Terms | Yes |

---

### Scenario 5: Register a Pharmacist (Togo)

| Step | Field | Value |
|------|-------|-------|
| 1 — Account Type | User Type | Pharmacist |
| 2 — Basic Info | Full Name | Kossi Agbemadon |
| | Email | kossi.agbemadon@test.tg |
| | Password | TestPharma123! |
| | Confirm Password | TestPharma123! |
| | Phone | +228 90 12 34 56 |
| | Date of Birth | 1988-01-10 |
| | Gender | Male |
| | Address | Boulevard du 13 Janvier, Lome, Togo |
| | Region | Togo |
| | Profile Picture | Skip (test without profile photo) |
| 3 — Documents | National ID/Passport (required) | Upload `docs/test-documents/national-id.svg` |
| | Pharmacy Degree (required) | Upload `docs/test-documents/medical-degree.svg` |
| | Professional License (required) | Upload `docs/test-documents/professional-license.svg` |
| | Registration Certificate (required) | Upload `docs/test-documents/work-certificate.svg` |
| | Pharmacy Affiliation Proof (required) | Upload `docs/test-documents/work-certificate.svg` |
| 4 — Verify & Submit | Accept Terms | Yes |

---

### Scenario 6: Register a Lab Technician (Rwanda)

| Step | Field | Value |
|------|-------|-------|
| 1 — Account Type | User Type | Lab Technician |
| 2 — Basic Info | Full Name | Diane Uwimana |
| | Email | diane.uwimana@test.rw |
| | Password | TestLab123! |
| | Confirm Password | TestLab123! |
...
| | Profile Picture | Upload `docs/test-documents/profile-photo.svg` |
| 3 — Documents | National ID/Passport (required) | Upload `docs/test-documents/national-id.svg` |
| | Laboratory Science Degree (required) | Upload `docs/test-documents/medical-degree.svg` |
| | Professional License (required) | Upload `docs/test-documents/professional-license.svg` |
| | Laboratory Accreditation (required) | Upload `docs/test-documents/work-certificate.svg` |
| | Proof of Employment (required) | Upload `docs/test-documents/work-certificate.svg` |
| 4 — Verify & Submit | Accept Terms | Yes |

---

### Scenario 7: Register an Emergency Worker (Benin)

| Step | Field | Value |
|------|-------|-------|
| 1 — Account Type | User Type | Emergency Worker |
| 2 — Basic Info | Full Name | Thierry Gnonlonfoun |
| | Email | thierry.g@test.bj |
| | Password | TestEMW123! |
| | Confirm Password | TestEMW123! |
...
| | Profile Picture | Upload `docs/test-documents/profile-photo.svg` |
| 3 — Documents | National ID/Passport (required) | Upload `docs/test-documents/national-id.svg` |
| | EMT/Paramedic Certification (required) | Upload `docs/test-documents/professional-license.svg` |
| | Professional License (optional) | Skip |
| | First Aid/ALS Certification (required) | Upload `docs/test-documents/work-certificate.svg` |
| | Proof of Employment (required) | Upload `docs/test-documents/work-certificate.svg` |
| 4 — Verify & Submit | Accept Terms | Yes |

---

### Scenario 8: Register with "Skip Documents" flow

| Step | Field | Value |
|------|-------|-------|
| 1 — Account Type | User Type | Patient |
| 2 — Basic Info | Full Name | Skip Documents Test |
| | Email | skip.test@test.com |
| | Password | SkipTest123! |
| | Confirm Password | SkipTest123! |
| | Phone | +230 5777 0000 |
...
| 3 — Documents | National ID/Passport | Click "I'll provide this later" |
| | Proof of Address | Click "I'll provide this later" |
| 4 — Verify & Submit | Accept Terms | Yes |

**After registration:** Login → Profile → Documents tab should be empty. Upload documents from profile page.

---

## Fake Test Documents

Located in `docs/test-documents/`. Upload these during registration or from the profile page.

| File | Use For | Format |
|------|---------|--------|
| `profile-photo.svg` | Profile picture upload | SVG (avatar illustration) |
| `national-id.svg` | National ID / Passport | SVG (ID card layout) |
| `proof-of-address.svg` | Proof of Address / Utility Bill | SVG (CEB electricity bill) |
| `insurance-card.svg` | Health Insurance Card | SVG (Swan Life insurance card) |
| `medical-degree.svg` | Medical Degree, Nursing Degree, Pharmacy Degree, Lab Degree | SVG (university diploma) |
| `professional-license.svg` | Professional License (any type) | SVG (Medical Council license) |
| `police-clearance.svg` | Police Clearance Certificate | SVG (character certificate) |
| `work-certificate.svg` | Work Certificate, Registration Certificate, Employment Proof, any other | SVG (hospital employment letter) |

> **Tip:** The signup form accepts `.pdf, .jpg, .jpeg, .png` and `.svg` files. All test documents are SVG format and work for any document slot.
...
| Region | Users | Regional Admin |
|--------|:-----:|:--------------:|
| Mauritius | 29 | 2 (Hassan, Vikash) |
| Madagascar | 11 | 1 (Tiana) |
| Kenya | 11 | 1 (James) |
| Togo | 10 | — |
| Benin | 10 | — |
| Rwanda | 10 | — |
...
> Plus 1 Super Admin created from environment variables = **82 users total**.
